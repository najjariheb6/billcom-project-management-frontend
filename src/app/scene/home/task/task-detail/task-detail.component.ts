...
@Component({
    selector: 'app-task-detail', templateUrl: './task-detail.component.html', styleUrls: ['./task-detail.component.scss']
})
export class TaskDetailComponent implements OnInit {
    ...
    onSubmit = (id: number) => {
        ...
        if (this.commentForm.invalid) {
            ...
            if (!this.update) {
                const comment = new Comment(this.commentForm.value.content, this.employee);
                //  let taskId = this.route.snapshot.params.id;
                this.taskService.addCommentToTask(this.id, comment).subscribe(data => {
                ...
                if (this.update) {
                    this.update = false;
                    const comment = new Comment(this.commentForm.value.content,

                        this.employee);

                    this.taskService.setCommentOfTask(this.commentId, comment).subscribe(data => {
                        this.commentForm.reset();
    ...
    getAllComment = () => {
        ...
        this.commentService.getTasksComment(this.id).subscribe((data: Comment[]) => {
            this.comments = data;
        });
    }
    ...
    formatDate = (d: Date) => moment(d).format('DD MMMM,  ');

    formathoure = (d: Date) => moment(d).format('hh:mm a');

    getsize = (c: Comment[]) => c.length;
    ...
    onUploadFiles(files: File[]): void {
        ...
        this.taskService.upload(this.id, formData).subscribe(event => {
