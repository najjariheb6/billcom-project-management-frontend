...
export class AttemptFirstLoginComponent implements OnInit {
  ...
  constructor(private _formBuilder: FormBuilder, private authService: AuthService, private _router: Router
    , private subjectService: SubjectInServiceService,private  notifyService : NotficationService) { }
  ...
  onSubmit() {
    ...
    if (this.updatePasswordForm.invalid) {
      ...
      const passwordUpdateModel = new PasswordUpdate(
       this.data[0],
       this.updatePasswordForm.value.password,
       this.updatePasswordForm.value.newPassword,

    )
    this.authService.updatePasswordAttemptFirstLogin(passwordUpdateModel).subscribe(
      data => {
