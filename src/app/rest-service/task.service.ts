...
export class TaskService {
    ...
    public update(idTask: any, data: any): Observable<any> {
        return this.httpClient.put(environment.urlConfig + `task/updateStatusTask/${idTask}`, data).pipe(tap(() => {
            this.refresh$.next();
        }));
    }
    ...
    public deleteTask(idTask: number): Observable<Task[]> {
        return this.httpClient.delete<any>(environment.urlConfig + `task/deleteTask/${idTask}`).pipe(tap(() => {
            this.refresh$.next();
        }));
    }
    ...
    public deleteComment(idComment: number): Observable<any> {
        return this.httpClient.delete<any>(environment.urlConfig + `task/deleteComment/${idComment}`).pipe(tap(() => {
            this.refresh.next();
        }));
    }
    ...
    public setCommentOfTask(idComment: number, comment: Comment): Observable<any> {
        return this.httpClient.put(environment.urlConfig + `task/setCommentOfTask/${idComment}`, comment).pipe(tap(() => {
            this.refresh.next();
        }));
    }

    public getTasksBeetweenDates(idTask: number, startedDate: string, finalDate: string): Observable<any> {
        return this.httpClient.get<any>(environment.urlConfig + `task/getTaskEnterDate/${idTask}`, {
            params: {
                startedDate: startedDate, finalDate: finalDate
            }
        });
    }
    ...
    public teamMembersTaskTwoDate(idTeamMember: number, startedDate: string, finalDate: string): Observable<any> {
        return this.httpClient.get<any>(environment.urlConfig + `task/getTeamMembersTasksTwoDate/${idTeamMember}`, {
            params: {
                startedDate: startedDate, finalDate: finalDate
            }
        });
